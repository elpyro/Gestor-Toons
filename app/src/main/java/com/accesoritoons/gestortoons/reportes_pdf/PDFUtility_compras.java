...
final class PDFUtility_compras
{
    ...
    void createPdf(@NonNull Context mContext, OnDocumentClose mCallback, ArrayList<Modelo_registro_compras> items, String total_costos, String total_ventas, String total_ganancias, @NonNull String filePath, boolean isPortrait) throws Exception
    {
        if(filePath.equals(""))
        {
        ...
        Paragraph temp = new Paragraph("Total compras bodega: "+formatoImporte.format(total_inventario), FONT_TITLE);
...
//
...

            {
                ...
                Paragraph temp = new Paragraph(mContext.getString(R.string.Compras_bodega), FONT_TITLE);
                temp.setAlignment(Element.ALIGN_CENTER);
                cell.addElement(temp);
    ...
    public PdfPTable createDataTable(Context mContext, ArrayList<Modelo_registro_compras> dataTable) throws DocumentException
    {
        ...
        table1.setWidths(new float[]{0.6f,0.4f,1.5F,0.4F,0.4F,1F});
        ...
        String cliente_diferente=dataTable.get(0).getId_referencia_vendedor();
        ...
        for (int i = 0; i < size; i++)
        {
            ...
            //total de cliente
            if(!cliente_diferente.equals(dataTable.get(i).getId_referencia_vendedor())){
                ...
                cliente_diferente=dataTable.get(i).getId_referencia_vendedor();
            }

            cell = new PdfPCell(new Phrase(dataTable.get(i).getFecha().substring(0,10), FONT_CELL));
            cell.setHorizontalAlignment(Element.ALIGN_CENTER);
            cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
            ...
            cell = new PdfPCell(new Phrase(dataTable.get(i).getId_pedido().substring(0,5), FONT_CELL));
            cell.setHorizontalAlignment(Element.ALIGN_CENTER);
            cell.setVerticalAlignment(Element.ALIGN_MIDDLE);
            cell.setPaddingLeft(left_right_Padding);
            cell.setPaddingRight(left_right_Padding);
